...
//        toPosts(page.getContent().stream().filter(line->postlist.contains(line.getId())).collect(Collectors.toList()));
		return new PageImpl<>(toPosts(page.getContent().stream().filter(line->postlist.contains(line.getId())).collect(Collectors.toList())),
...
//        postRepository.save(po);
    ...
    public void addvote(long userId, long postId) {
        favoriteService.addvote(userId, postId);
    }

    @Override
    public List<Vote> selectvote(long postId) {
